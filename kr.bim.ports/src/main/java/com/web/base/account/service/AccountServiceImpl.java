...
import com.web.base.account.dao.UserPhotoDao;
import com.web.base.account.dao.UserPhotoDto;
...
public class AccountServiceImpl extends BaseService implements IAccountService {
	...
	public UserDTO detail(String paramUserID) {
		...
		try {
			...
			//	rs.setUser_phone(AES256Cipher.AES_Decode(rs.getUser_phone(), rs.getUser_key()));
				...
				UserPhotoDto userPhotoDto=userPhotoDao.detail(paramMap);
				if(userPhotoDto!=null) {
					AttachFileDTO attachFileDTO =fileService.detail(userPhotoDto.getFile_seq());
					rs.setUserPhotoImage(fileService.getImageBase64(attachFileDTO));
	...
	public UserDTO detail(String paramUserID, String paramPwd) {
		...
		try {
			...
			paramMap.put("user_pwd", EgovFileScrty.encryptPassword(paramPwd,paramUserID));
...
public int update(UserDTO paramBean, CommonParams comm) {
	...
	@Override
	public int updatePwd(UserDTO paramBean, CommonParams comm) {
		int rs = 0;
		try{
			HashMap<String,Object> paramMap=new HashMap<String, Object>();
			paramMap.put("user_id", paramBean.getUser_id());
			...
			String newPassword=EgovFileScrty.encryptPassword(paramBean.getUser_pwd(),paramBean.getUser_id());
			UserDTO userEntity = userDao.detail(paramMap);
			if (userEntity != null) {
				MberManageVO mberManageVO=mberManageService.selectMberByID(paramBean.getUser_id());
				mberManageVO.setPassword(newPassword);
				mberManageService.updatePassword(mberManageVO);
	...
	@Override
	public int createPhoto(CommonParams commons, String paramUserId, int paramFileSeq) {
		int rs=0;
		if(paramFileSeq<=0) return rs;
		AttachFileDTO fileInfo= fileService.detail(paramFileSeq);
		if(fileInfo==null) return rs;
		
		HashMap<String,Object> paramMap=new HashMap<String, Object>();
		paramMap.put("user_id",paramUserId);
		paramMap.put("create_user_id",commons.getLoginUserID());
		paramMap.put("file_seq",paramFileSeq);
		
		rs+=userPhotoDao.delete(paramMap);
		rs+=userPhotoDao.create(paramMap);
		return rs;
	}
