...
import com.web.base.FileViewModel;
...
public class AdminFileController extends BaseController {
	...
	@RequestMapping(value = "/file/drop/{fileSeq}", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/api/file/delete", method = RequestMethod.POST)
	@ResponseBody
	public FileViewModel  deleteFileAction(@RequestBody FileViewModel vm,Model model) throws Exception {
		List<Integer> listSeqs=new ArrayList<Integer>();
		for (AttachFileDTO attachFileDTO : vm.getListFiles()) {
			listSeqs.add(attachFileDTO.getFile_seq());
		}
		
		int cnt=fileService.delete(listSeqs);
		if(cnt>0) {
			vm.setResultCount(cnt);
			vm.setListFiles(fileService.list(vm.getListFiles().get(0).getFile_grp_id()));
		}
		return vm;
	}
	...
	public HashMap<String, String> createFile(BaseViewModel vm, ModelMap model)
		...
		if(vm.getFileGroup()==null || vm.getFileGroup().equals("")){
			rs = fileService.create(vm.getUploadFiles(),getCommonParams());			
		}else{			
