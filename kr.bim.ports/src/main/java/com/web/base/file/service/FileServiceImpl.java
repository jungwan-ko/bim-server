...
import com.web.base.file.dao.IFileDao;
...
public class FileServiceImpl extends BaseService implements IFileService {
	...
	public String editFile(List<MultipartFile> paramFiles,String fileGrpID) {
		...
		List<AttachFileDTO> rs = createFile(paramFiles, tempPath, fileGrpID);
		for (AttachFileDTO attachFileDTO : rs) {
			fileDao.create(attachFileDTO);
		}
	...
	public String create(List<MultipartFile> paramFiles,CommonParams common, String filePath) {
		...
		for (AttachFileDTO attachFileDTO : rs) {
			attachFileDTO.setCreate_user_id(common.getLoginUserID());
			attachFileDTO.setUpdate_user_id(common.getLoginUserID());
			fileDao.create(attachFileDTO);
		}
	...
	public int delete(int paramSeq) {
		...
		return fileDao.delete(paramMap);
	}
	...
	public String create(List<MultipartFile> paramFiles,CommonParams common) {
		...
		return create(paramFiles,common,tempPath);
	}
	...
	public int updateFileGrp(String file_grp_id_now, String fileGrpID) {
		HashMap<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("oldGrp", file_grp_id_now);
		paramMap.put("newGrp", fileGrpID);
		return fileDao.updateFileGrp(paramMap);
	}

	@Override
	public int delete(List<Integer> paramSeqs) {
		int rs=0;
		for (Integer integer : paramSeqs) {
			rs+=delete(integer);
		}
		return rs;
	}
	...
	public List<AttachFileDTO> list(String fileGrp) {
		...
		paramMap.put("file_grp_id", fileGrp);
		List<AttachFileDTO> rs = fileDao.list(paramMap);
		try {
			for (AttachFileDTO attachFileDTO : rs) {
				int n=SysKeyword.EXT_IMG.indexOf(attachFileDTO.getFile_ext());
